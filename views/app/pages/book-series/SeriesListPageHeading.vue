  ...
  <b-row>
    <b-colxx xxs="12">
      ...
      <div class="top-right-button-container">
        <b-button @click="gotoAddSery" variant="primary" size="lg" class="top-right-button">{{ $t('pages.add-new')
        }}</b-button>
      </div>
        ...
        <b-collapse id="displayOptions" class="d-md-block">
          <div class="float-md-right pt-1">
            ...
            <b-dropdown id="ddown2" right :text="`${perPage}`" variant="outline-dark" class="d-inline-block" size="xs">
              <b-dropdown-item v-for="(size, index) in pageSizes" :key="index" @click="changePageSize(size)">{{ size
              }}</b-dropdown-item>
            </b-dropdown>
          </div>
        </b-collapse>
...
export default {
  ...
  methods: {
    gotoAddSery() {
      this.$router.push(`${adminRoot}/book-series/add`);
    }
  }
};
</script>
