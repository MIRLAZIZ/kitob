...
import BooktypeListPageListing from "./BooktypeListPageListing";

export default {
  components: {
    "list-page-heading": BooktypeListPageHeading,
    "list-page-listing": BooktypeListPageListing
  },
  data() {
    return {
      isLoad: false,
  ...
  methods: {
    ...mapActions(['getBooktypeList','destroyFaq']),
    ...
    async getItemsAndSet(){
      await this.getBooktypeList('?page='+this.page);
      this.total = this.booktypeList.result.total;
      this.from = this.booktypeList.result.from;
      this.to = this.booktypeList.result.to;
      this.items = this.booktypeList.result.data.map(x => {
  ...
  computed: {
    ...mapGetters(['booktypeList','getFaqAlertMsg']),
    isSelectedAll() {
