  ...
  <div>
    ...
    <b-row>
        <b-colxx xxs="12">
                ...
                <b-row>
                    <div class="col-md-6">
                        <div class="list-group">
                            <div class="list-group-item">Name: <span class="text-primary">{{invoiceFrom.user ? invoiceFrom.user.name : 'No name'}}</span></div>
                            <div class="list-group-item">Email: {{invoiceFrom.user ? invoiceFrom.user.email : 'no email'}}</div>
                            <div class="list-group-item">Phone: {{invoiceFrom.user ? invoiceFrom.user.phone : 'no phone'}}</div>
                            <div class="list-group-item">Address: {{invoiceFrom.user ? invoiceFrom.user.address : 'no address'}}</div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="list-group text-right">
                            ...
                            <div class="">
                                ...
                                ><i class="iconsminds-checkout"></i> {{ $t('pages.create-fargo-order') }}</b-button>
                            </div>
                        </div>
                    </div>
                </b-row>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td></td>
                                    <td><strong>{{numberFormat(invoiceFrom.total_amount)}}</strong></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <b-row>
                    <div class="col-md-12">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr v-for="(ship_cost,key) in invoiceFrom.shippingcosts" :key="key">
                                    <td>{{ship_cost.name}}</td>
                                    <td>{{ship_cost.type}}</td>
                                    <td>{{ship_cost.weight}}</td>
                                    <td class="text-upppercase">
                                        <b-badge pill v-if="ship_cost.status == 'draft'" variant="warning" >
                                            {{ ship_cost.status }}
                                        </b-badge>
                                        <b-badge pill v-if="ship_cost.status == 'canceled'" variant="danger" >
                                            {{ ship_cost.status }}
                                        </b-badge>
                                        <b-badge pill v-if="ship_cost.status == 'success'" variant="success" >
                                            {{ ship_cost.status }}
                                        </b-badge>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td></td>
                                    <td></td>
                                    <td><strong>{{numberFormat(getShippingCostsTotal())}}</strong></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <b-card class="mb-4" :title="$t('menu.order-details')" v-if="invoiceFrom.fargo">
                <b-row>
                    <div class="col-md-12">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr v-for="(order,index) in invoiceFrom.fargo" :key="index">
                                    ...
                                    <td>
                                        <b-badge pill v-if="order.status == 'canceled'" variant="danger" >
                                            {{ order.status }}
                                        </b-badge>
                                        <b-badge pill v-if="order.status == 'active'" variant="success" >
                                            {{ order.status }}
                                        </b-badge>
                                    </td>
                                    <td>
                                        <button class="btn btn-sm btn-primary" @click="checkFargoOrder(order.order_number)">
                                            <i class="iconsminds-24-hour"></i>
                                            Check Status
                                        </button>
                                        <button class="btn btn-sm btn-danger" @click="cancelFargoOrder(order.order_id)">
                                            <i class="iconsminds-delete-file"></i>
                                            Cancel
                                        </button>
                                        <button class="btn btn-sm btn-success" @click="infoFargoOrder(order.data)">
    ...
    <b-modal ref="my-modal" size="lg" hide-footer title="Fargo order statuses">
      <div class="d-block text-center">
        <table class="table table-bordered table-striped" v-if="orderStatuses.total > 0">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Status</th>
    ...
    <b-modal ref="order-data-modal" size="xl" hide-footer title="Fargo order details">
...
export default {
    ...
    async mounted(){
        ...
        this.invoiceFrom = this.getCurrentOrder;
        this.selected = this.invoiceFrom.status;
    },
};
</script>
